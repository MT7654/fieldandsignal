...
function ResearchResults({ state }: { state: SecondaryResearchState }) {
  ...
  return <>
    ...
    <div className="research-dashboard">
      <section className="panel real-source-library"><div className="panel-header"><div className="maya-identity compact"><AgentAvatar slug="maya-chen"/><div><Badge>Live, relevance-filtered web research</Badge><h2>Verified source library</h2></div></div><Search size={18}/></div>
      ...
      <aside className="research-sidebar">
        ...
        <section className="panel usage-panel"><h2>Research usage</h2><dl><div><dt>Search requests</dt><dd>{state.usage.searchRequests}/{SECONDARY_RESEARCH_LIMITS.queries}</dd></div><div><dt>Page fetches</dt><dd>{state.usage.pageFetches}/{SECONDARY_RESEARCH_LIMITS.pageFetches}</dd></div><div><dt>Sources retained</dt><dd>{state.sources.length}/{SECONDARY_RESEARCH_LIMITS.sources}</dd></div><div><dt>Qwen calls</dt><dd>{state.usage.llmCalls}/1</dd></div><div><dt>Estimated Qwen input</dt><dd>{state.usage.estimatedInputTokens.toLocaleString()} tokens</dd></div></dl><small>Search and page retrieval do not use Hugging Face tokens. Qwen is called at most once for compact synthesis.</small></section>

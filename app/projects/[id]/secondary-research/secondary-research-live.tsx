...
import type { SecondaryResearchState } from "@/lib/secondary-research-types";
...
export function LiveSecondaryResearch() {
  ...
  const run = useCallback(async () => {
    ...
    try {
      ...
      for (let step = 0; current && current.status === "in_progress" && step < 24; step++) {
  ...
  const actions = state?.status === "complete" ? <Button href={state.researchMode === "secondary" ? "/projects/live/analysis" : "/projects/live/survey"}>{state.researchMode === "secondary" ? "Continue to analysis" : "Continue to primary research"}</Button> : undefined;
...
function MayaResearchProgress({ state, progress, running }: { state?: SecondaryResearchState; progress: number; running: boolean }) {
  ...
  return <section className="maya-progress" aria-live="polite" aria-busy={running}>
    ...
    <div className="maya-live-metrics"><div><strong>{searched}/{state?.queries.length ?? "—"}</strong><span>Searches completed</span></div><div><strong>{state?.candidates.length ?? 0}</strong><span>Candidates found</span></div><div><strong>{reviewed}</strong><span>Pages inspected</span></div><div><strong>{state?.sources.length ?? 0}</strong><span>Sources retained</span></div></div>
    <small>Results appear as they are verified. Keep this page open; if it closes, Maya resumes from the last saved step.</small>
  </section>;
}
...
function ResearchResults({ state }: { state: SecondaryResearchState }) {
  ...
  const dataPoints = state.sources.flatMap((source) => source.dataPoints.map((value) => ({ value, sourceId: source.id, workstream: source.workstream }))).slice(0, 6);
  ...
  return <>
    ...
    <div className="research-dashboard">
      ...
      <aside className="research-sidebar">
        ...
        <section className="panel usage-panel"><h2>Research usage</h2><dl><div><dt>Search requests</dt><dd>{state.usage.searchRequests}/4</dd></div><div><dt>Page fetches</dt><dd>{state.usage.pageFetches}/8</dd></div><div><dt>Qwen calls</dt><dd>{state.usage.llmCalls}/1</dd></div><div><dt>Estimated Qwen input</dt><dd>{state.usage.estimatedInputTokens.toLocaleString()} tokens</dd></div></dl><small>Search and page retrieval use Oxylabs, not Hugging Face tokens. Qwen is called at most once for compact synthesis.</small></section>

  ...
  return <>
    ...
    <div className="research-dashboard">
      ...
      <aside className="research-sidebar">
        ...
        <section className="panel usage-panel"><h2>Research usage</h2><dl><div><dt>Web searches</dt><dd>{state.usage.searchRequests}/{Math.max(SECONDARY_RESEARCH_LIMITS.queries, state.queries.length)}</dd></div><div><dt>Page fetches</dt><dd>{state.usage.pageFetches}/{Math.max(SECONDARY_RESEARCH_LIMITS.pageFetches, state.usage.pageFetches)}</dd></div><div><dt>Sources retained</dt><dd>{state.sources.length}/{Math.max(SECONDARY_RESEARCH_LIMITS.sources, state.sources.length)}</dd></div><div><dt>Synthesis calls</dt><dd>{state.usage.llmCalls}/1</dd></div><div><dt>Estimated synthesis input</dt><dd>{state.usage.estimatedInputTokens.toLocaleString()} tokens</dd></div></dl><small>GPT-5.6 Luna performs focused source discovery and is called once more for compact synthesis. Page extraction is deterministic and does not use model tokens.</small></section>
